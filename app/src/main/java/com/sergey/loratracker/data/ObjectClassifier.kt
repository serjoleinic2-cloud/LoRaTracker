...
enum class DetectedObject(
    ...
    HUMAN("Человек", "\uD83D\uDEB6", 85f..350f, 1.2f, 0f, 50f, "Голос"),
    GROUP("Группа", "\uD83D\uDC65", 150f..500f, 1.3f, 0f, 80f, "Несколько человек"),
    CAR("Легковая", "\uD83D\uDE97", 200f..600f, 1.5f, 0f, 150f, "Двигатель"),
    ...
    TANK("Танк", "\uD83D\uDEA2", 100f..400f, 1.3f, 0f, 1000f, "Гусеницы"),
    TRACTOR("Трактор", "\uD83D\uDE9C", 80f..200f, 0.9f, 0f, 300f, "Строительная техника"),
    CONSTRUCTION("Стройка", "\uD83C\uDFD7", 80f..300f, 1.0f, 0f, 200f, "Техника"),
    BICYCLE("Велосипед", "\uD83D\uDEB2", 50f..120f, 1.1f, 0f, 30f, "Цепь"),
    LOW_FREQ("Низкочастотный", "\uD83D\uDD0A", 150f..1000f, 0.5f, 0f, 100f, "Мотор/гул"),
    ...
    companion object {
        fun classify(packet: TelemetryPacket): DetectionResult {
            ...
            FileLogger.d("CLASSIFY_ENTER", "classify called: peak=$peak ratio=$ratio")
            ...
            if (!AdaptiveThreshold.isCalibrated() && AdaptiveThreshold.isTimeout()) {
                AdaptiveThreshold.forceCalibrate()
            }
            ...
            if (peak < 150f && UsbSerialService.samePeakCount > 10) {
                return DetectionResult(false, 0f, null, SoundLevel.LOW, UNKNOWN, 0f, "ШУМ МИКРОФОНА ${peak.toInt()}Hz")
            }

            // Проверка гармоничности — дрон имеет чёткие гармоники
            ...
            if (peak in 8000f..20000f && hasHarmonics && ratio >= threshold * 0.8f) {
                ...
                return DetectionResult(
                    ...
                    rmsDb = 0f,
                    reason = "ДРОН | ${peak.toInt()}Hz | гармоники ${"%.1f".format(harmonicRatio)}x"
                )
            }
            ...
            val scores = values().filter { it != UNKNOWN }.map { obj ->
                ...
                val ratioScore = if (obj.centroidMinRatio > 0f) {
                    if (ratio >= obj.centroidMinRatio)
                        (ratio / obj.centroidMinRatio).coerceAtMost(2f)
                    else
                        ratio / obj.centroidMinRatio
                } else 1f
